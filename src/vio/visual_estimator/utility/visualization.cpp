...
void pubLatestOdometry(const Eigen::Vector3d &P, const Eigen::Quaterniond &Q, const Eigen::Vector3d &V, const std_msgs::Header &header, const int& failureId)
{
    ...
    // add by YJZ
    tf::Transform q_odom_imu = tf::Transform(tf::Quaternion(Q.x(), Q.y(), Q.z(), Q.w()), tf::Vector3(P.x(), P.y(), P.z()));
    tf::Transform q_odom_lidar = q_odom_imu * lidar_to_imu_transform.inverse();

    tf::quaternionTFToMsg(q_odom_lidar.getRotation(), odometry.pose.pose.orientation);
    ...
    // for visualize
    tf::Transform t_w_lidar = tf::Transform(q_odom_lidar.getRotation(), tf::Vector3(q_odom_lidar.getOrigin().getX(), q_odom_lidar.getOrigin().getY(), q_odom_lidar.getOrigin().getZ()));
    tf::StampedTransform trans_world_lidar_ros = tf::StampedTransform(t_w_lidar, header.stamp, "vins_world", "vins_lidar_ros");
    ...
    if (ALIGN_CAMERA_LIDAR_COORDINATE)
    {
        ...
        if (header.stamp.toSec() - last_align_time > 1.0)
        {
            try
            {
                ...
                t_odom_world = transformConversion(trans_odom_baselink) * transformConversion(trans_world_lidar_ros).inverse();

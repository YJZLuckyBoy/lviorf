...
public:
    ...
    n(n_in)
    {
        ...
        //                 0, -1, 0,
        //                 0,  0, 1]
        // extrinsic world_lio frame to world_vio frame
        // q_lidar_to_cam_eigen = Eigen::Quaterniond(0, 0, 0, 1); // rotate position by pi, (w, x, y, z) // mark: camera - lidar
        q_lidar_to_cam = tf::Transform(tf::Quaternion(0, 0, 1, 0), tf::Vector3(0., 0., 0.));
        // lidar_to_imu_transform = tf::Transform(lcExtrinsicRot_, tf::Vector3(lcExtrinsicTrans_[0], lcExtrinsicTrans_[1], lcExtrinsicTrans_[2]));
    ...
    vector<float> getOdometry(deque<nav_msgs::Odometry>& odomQueue, double img_time)
    {
        ...
        // transform lidar pose from world_lio to world_vio and caculate camera pose in world_vio frame
        tf::Transform lidar_pose = tf::Transform(q_odom_lidar, tf::Vector3(odomCur.pose.pose.position.x, odomCur.pose.pose.position.y, odomCur.pose.pose.position.z));
        tf::Transform lidar_vel = tf::Transform(q_odom_lidar, tf::Vector3(odomCur.twist.twist.linear.x, odomCur.twist.twist.linear.y, odomCur.twist.twist.linear.z));
        tf::Transform imu_pose = lidar_pose * lidar_to_imu_transform;
        tf::Transform imu_vel = lidar_vel * lidar_to_imu_transform;
        tf::Transform world_vio_to_imu_pose = q_lidar_to_cam * imu_pose;
        tf::Transform world_vio_to_imu_vel = q_lidar_to_cam * imu_vel;
        ...
        // tf::quaternionTFToMsg(camera_pose, odomCur.pose.pose.orientation);
        tf::quaternionTFToMsg(world_vio_to_imu_pose.getRotation(), odomCur.pose.pose.orientation);
        ...
        Eigen::Vector3d p_eigen_new(world_vio_to_imu_pose.getOrigin().getX(), world_vio_to_imu_pose.getOrigin().getY(), world_vio_to_imu_pose.getOrigin().getZ());
        Eigen::Vector3d v_eigen_new(world_vio_to_imu_vel.getOrigin().getX(), world_vio_to_imu_vel.getOrigin().getY(), world_vio_to_imu_vel.getOrigin().getZ());

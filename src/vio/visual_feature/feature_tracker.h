...
public:
                                          ...
                                          const vector<geometry_msgs::Point32>& features_2d)
    {
        ...
        for (int i = 0; i < (int)features_2d.size(); ++i)
        {
            ...
            Eigen::Vector3d feature_cur(features_2d[i].x, features_2d[i].y, features_2d[i].z); // z always equal to 1
            feature_cur.normalize(); 

            Eigen::Vector3d feature_trans = camera_to_ros_matrix.inverse() * feature_cur;
...
/*                 Eigen::Vector3d p_3d(-depth_cloud_local->points[i].y,
                ...
                Eigen::Vector3d p_3d = camera_to_ros_matrix * p_3d_;
